...
def getCookiesOfGoogle():
    ...
    res = requests.get(
        'https://accounts.google.com/signin/v2/usernamerecovery?flowName=GlifWebSignIn&flowEntry=ServiceLogin&hl=en-GB', headers=heasers)
    fr = re.search(
        r'data-initial-setup-data="%.@.null,null,null,null,null,null,null,null,null,&quot;(.*?)&quot;,null,null,null,&quot;(.*?)&', res.text).group(2)
    try:
...
def gmail_call_n1(username):
    ...
    response = requests.post(
        'https://accounts.google.com/_/lookup/accountlookup', headers=headers, params=params, data=data)
...
def gmail_call_n2(username):
    ...
    response = requests.post(
        'https://accounts.google.com/_/signup/webusernameavailability', headers=headers, params=params, data=data)
