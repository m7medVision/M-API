...
def check_username(username):
    ...
    res = requests.get(
        'https://api.brandsnag.com/api/social-media/username-available-by-source?username={}&source=tiktok'.format(username), headers=headers)
    try:
        json_res = res.json()
        if json_res['success']:
...
def get_last_video_id(username):
    ...
    response = requests.get(
        'https://www.tiktok.com/@{}'.format(username), headers=headers)

    soup = BeautifulSoup(response.text, 'lxml')

    lastvideoid = ast.literal_eval("["+re.search(r'"ItemList":{"user-post":{"list":\[(.*?)\]', soup.find_all(
        'script')[5].text.strip().replace('window[\'SIGI_STATE\']=', '')).group(1)+"]")[0]
    return lastvideoid
...
def get_account_info(username):
    ...
    script_codes = soup.find_all('script')[5].text.strip().replace(
        'window[\'SIGI_STATE\']=', '')
    ...
    some_video_ids = ast.literal_eval("["+re.search(r'"ItemList":{"user-post":{"list":\[(.*?)\]', soup.find_all(
        'script')[5].text.strip().replace('window[\'SIGI_STATE\']=', '')).group(1)+"]")
