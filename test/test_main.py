...
def test_youtube():
    ...
    response = client.get("/api/yt?url=https%3A%2F%2Fwww.youtube.com%2Fwatch%3Fv%3Duc6vV8o_aAU")
    assert response.json()['status'] == "ok"

def test_tiktok_downloader():
    """This test will check if the tiktok downloader is working"""
    response = client.get("/api/tk?url=https://www.tiktok.com/@majhc/video/7049993308386495745")
    assert response.json()["success"]

def test_twitter_downloader():
    """This test will check if the twitter downloader is working"""
    response = client.get("/api/twitter?url=https%3A%2F%2Ftwitter.com%2FOmanTVGeneral%2Fstatus%2F1478376146907668483%3Fs%3D20")
    assert response.json()['status'] == "ok"
...
def test_auto():
    ...
    response = client.get("/api/downloader/auto?url=https%3A%2F%2Fwww.youtube.com%2Fwatch%3Fv%3Duc6vV8o_aAU")
    assert response.json()['status'] == "ok"
...
def test_google_mail_checker():
    ...
    response = client.get("/api/email/checker/google?email=oman4omani%40gmail.com")
    assert response.json()['status'] == "success"

def test_microsoft_mail_checker():
    """This test will check if the gmail mail checker is working"""
    response = client.get("/api/email/checker/microsoft?email=oman4omani%40outlook.com")
    assert response.json()['status'] == "success"
...
def test_get_last_video_youtube_by_id():
    ...
    response = client.get("/api/get_last_videoid_youtube?channel_id=UCnddj1RPMErfmyUMGULawZQ")
    assert response.json()['status'] == "success"

def test_get_last_video_youtube_by_username():
    """This test will check if the last video youtube is working"""
    response = client.get("/api/get_last_videoid_youtube_by_username?username=majhcc")
    assert response.json()['status'] == "success"
